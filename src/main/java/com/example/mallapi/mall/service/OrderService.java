...
public class OrderService {
    private final ItemRepository itemRepository;                    // 상품 정보 : 가격, 재고수량
    private final MemberRepository memberRepository;          // 주문 고객 정보
    private final OrderRepository orderRepository;                  // 주문 정보

    private final ItemImgRepository itemImgRepository;          // 주문상품목에 적용될 상품이미지 정보

    // 1. 상품 주문 서비스(주문상품 아이디, 수량, 현재 로그인 고객:주문 고객)
    public Long order(OrderDTO orderDTO, String email){
        ...
        // 1.4 주문상품 DTO(상품 상세페이지에서 주문할 상품 엔티티와 주문 수량:OrderDTO)로 통해 주문 상품 Entity객체 생성(orderItem Entity)하기
    ...
    public Page<OrderHistDTO> getOrderList(String email, Pageable pageable){
        ...
        for (Order order : orders){
            ...
            /*
            1. JPA N+1문제
            @OneToMany 등에서 하위엔티티들을 lazy loading으로 가져올 때마다 자식 조회 쿼리가 추가발생
            ...
            //--------------------------------------------------------------------------  //
            // 주문서 이력에 있는 주문상품 Entity List -> DTO에 전달 : N+1 문제 발행
            //-------------------------------------------------------------------------  //

            // 개별 주문서에 등록된 주문 상품 목록 추출
            ...
            for (OrderItem orderItem: orderItems){
                ...
                // 3. 개별 주문서에 화면에 보여줄  주문 상품 이력  DTO List구조에 등록
            ...
            // 현재 로그인한 고객의 모든 주문이력을  List객체에 주문이력 상품 추가: 주문서1, 주문서2,....
    ...
    public boolean validateOrder(Long orderId, String email){
        ...
        if (!StringUtils.equals(currMember.getEmail(), savedMember.getEmail()))
            return false;    // 일치하지 않으면 (현재 로그인 회원과 주문서에 있는 회원 이메일 동일하지 않으면 ) false반환
